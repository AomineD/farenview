package com.dagf.persoview.flipviewpager.view;
...
public class FlipViewPager extends FrameLayout {
    ...
    private void setFlipDistance(float flipDistance) {
     ...
     //   Log.e("MAIN", "setFlipDistance: "+mCurrentPageIndex);
    ...
    public void setAdapter(ListAdapter adapter, int activePage, int row, int maxItems) {
        ...
        BaseFlipAdapter.MergeAdapter mergeAdapter = (BaseFlipAdapter.MergeAdapter) adapter;
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
          ...
          //  if(item.pageView != null && mergeAdapter.noIsNull() && i < 3) {
                pages.put(i, item);
                //Log.e("MAIN", "setAdapter: pos "+mergeAdapter.noIsNull());

        }
      //  Log.e("MAIN", "setAdapter: COUNT = "+(adapter.getView(2, pages.get(2).pageView, this) != null));
